/* eslint-disable react/jsx-key */
/* eslint-disable no-unused-vars */
import React, { useEffect, useState } from 'react';
import Cart from '../Cart/Cart';
import { Slide, ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';

const Main = () => {

    const [cards, setCards] = useState([]);
    const [selectedCard, setSelectedCard] = useState([])
    const [remaining, setRemaining] = useState(0)
    const [totalCredit, setTotalCredit] = useState(0)
    const [totalPrice, setTotalPrice] = useState(0)

    useEffect(() => {
        fetch('./Cards.json')
            .then(res => res.json())
            .then(data => setCards(data))
    }, [])


    const handleCard = (card) => {
        const isSelected = selectedCard.find(item => item.id == card.id);
        let countCredit = card.credit;
        let totalCoursePrice = card.price;
        if (isSelected) {
            toast('already selected');


        } else {
            selectedCard.forEach((item) => {
                countCredit = countCredit + item.credit;
                totalCoursePrice = totalCoursePrice + item.price;
            })

            const remainingCredit = 20 - countCredit;


            if (countCredit > 20) {
                toast('Your credit limit is 20')
            } else {
                setTotalCredit(countCredit);
                setRemaining(remainingCredit);
                setSelectedCard([...selectedCard, card]);
                setTotalPrice()
                setTotalPrice(totalCoursePrice.toFixed(2));
            }

        }



    }

    return (
        <div className='lg:flex mx-12 mt-8 lg:justify-between flex-row gap-4'>
            <div className='grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5 '>
                {
                    cards.map(card => (
                        <div key={card.id} className=" bg-white border h-[460px] rounded-xl space-y-4 space-x-5">
                            {/* flex justify-center mt-8 */}
                            <figure>
                                <img src={card.img} alt="Shoes" className="rounded-xl w-full p-4" />
                            </figure>
                            <div className="card-body">
                                <h2 className="font-bold">{card.course_name}</h2>
                                <p className='text-[#1C1B1B99] mt-2 mb-4 text-lg'>{card.details}</p>
                                <div className="flex justify-between  text-[#1C1B1B99]">
                                    <div className='flex'>
                                        <svg className='mr-4' xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                            <path d="M12 1V23" stroke="#1C1B1B" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                            <path d="M17 5H9.5C8.57174 5 7.6815 5.36875 7.02513 6.02513C6.36875 6.6815 6 7.57174 6 8.5C6 9.42826 6.36875 10.3185 7.02513 10.9749C7.6815 11.6313 8.57174 12 9.5 12H14.5C15.4283 12 16.3185 12.3687 16.9749 13.0251C17.6313 13.6815 18 14.5717 18 15.5C18 16.4283 17.6313 17.3185 16.9749 17.9749C16.3185 18.6313 15.4283 19 14.5 19H6" stroke="#1C1B1B" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                        </svg>
                                        <p>Price: {card.price}</p>
                                    </div>
                                    <div className='flex'>
                                        <svg className='mr-4' xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                            <path d="M12 6.042C10.3516 4.56336 8.2144 3.74694 6 3.75C4.948 3.75 3.938 3.93 3 4.262V18.512C3.96362 18.172 4.97816 17.9989 6 18C8.305 18 10.408 18.867 12 20.292M12 6.042C13.6483 4.56328 15.7856 3.74685 18 3.75C19.052 3.75 20.062 3.93 21 4.262V18.512C20.0364 18.172 19.0218 17.9989 18 18C15.7856 17.9969 13.6484 18.8134 12 20.292M12 6.042V20.292" stroke="#1C1B1B" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                        </svg>
                                        <p className='pr-8'>Credit: {card.credit}hr</p>
                                    </div>
                                </div>
                                <div className='items-center mt-3 pl-2'>
                                    <button onClick={() => handleCard(card)} className=" h-10 rounded-lg btn bg-blue-500 text-white font-bold w-11/12">Select</button>
                                    <ToastContainer></ToastContainer>
                                </div>
                            </div>
                        </div>
                    ))
                }

            </div>
            <div className='md:mt-3 lg:mt-0 mt-3'>
                <Cart selectedCard={selectedCard} remaining={remaining} totalCredit={totalCredit} totalPrice={totalPrice}></Cart>
            </div>
        </div>
    );
};

export default Main;